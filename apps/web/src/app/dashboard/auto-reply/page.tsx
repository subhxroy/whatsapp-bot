  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('/api/auto-replies', {
        ...
        credentials: 'include',
        body: JSON.stringify({ trigger, matchType, response, priority, cooldown }),
      });
      setShowModal(false);
      setTrigger('');
      setResponse('');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-[#070607]/70 mt-1">
            Automated keyword triggers and contextual responses
          </p>
        </div>
        <button
                ...
                {rules.map((rule) => (
                  <tr key={rule.id} className="hover:bg-[#e2e2df]/50 transition">
                    ...
                    <td className="py-4 px-6">
                      <span className="rounded-full bg-[#f5f28e] px-3 py-1 text-xs font-semibold text-[#070607]">
                        {rule.matchType}
                      </span>
                    </td>
