...
4. **View-Once Media Handling & Auto-Reveal**:
   - Standard commands (`sticker`, `toimg`) respect view-once and reject conversion.
   - **Manual `.vv` / `.avv` command**: reveals a quoted view-once message inside the active conversation where the command was sent.
   - **Auto-view-once reveal (`handleAutoVv` in `dispatcher.ts`)**: Any incoming view-once message (image/video/audio) from any private chat or group is automatically decrypted and **forwarded strictly to the bot owner's private chat / self-DM ("Subhankar Roy (You)")** — never posted in the sender's conversation.
   - **Direct stream decryption fallback**: `downloadMediaFromContent` provides a direct `directPath` + `mediaKey` fallback via Baileys `downloadContentFromMessage` when WhatsApp CDN URLs expire.
   - **Full audit trail**: Every auto-vv trigger, successful forward, or error is written to Firestore audit logs (`AUTO_VV_TRIGGER`, `AUTO_VV_SUCCESS`, `AUTO_VV_DOWNLOAD_FAILED`, `AUTO_VV_SEND_FAILED`).
5. **Anti-Delete & Revoke Deduplication**:
   - When a sender deletes/revokes a message for everyone, Baileys emits either `messages.upsert` (ProtocolMessage REVOKE) or `messages.update` (StubType REVOKE).
   - A deduplication set (`processedRevokeIds`) in `WhatsAppClient` ensures each deletion is captured and persisted exactly once.
   - Recovered body content from the 300-message in-memory cache is preserved and visible in Dashboard → Deleted Messages (subject to `MESSAGE_CONTENT_RETENTION`).
6. **Monetization Gate**: WhatsApp connection (QR + pairing) is gated behind a **₹150 one-time UPI activation payment** + admin approval. Payment requests are stored in Firestore (`payments`), verified by an admin (dashboard Admin Portal or the standalone `admin/` portal). Access is granted only by DB role (`ADMIN`/`OWNER`) or an approved `payments` record. Admin users (`isAdminUser`) are auto-approved without payment.
7. **Scheduled Delivery Engine**: `.birthday` / `.schedule` (and the dashboard Schedule page → `POST /api/scheduled-messages`) queue messages in Firestore (`scheduledMessages`); a background scheduler in `apps/api` polls every **5 seconds** with **atomic claims** (PENDING→PROCESSING transition prevents double-sends across instances), delivers them at the scheduled minute through the sender's connected session, and writes 12 delivery event types (CREATED/SENT/FAILED/etc.) to the schedule event history.
8. **Zero Telemetry**: No third-party analytics, remote tracking, or cloud bot host dependencies beyond Firebase/Render/Netlify infra.
9. **Deployment Split**: Dashboard + static sites are hosted on Netlify; the API/bot runtime is hosted on Render; `docker-compose.yml` remains available for fully self-hosted deployments.
...
- **Actual method surface**:
  ...
  - `getConnectedPhone()`: returns the phone number of the currently connected WhatsApp account (digits-only, captured from Baileys `connection.update` JID or mapped from LID).
  - `getConnectedJid()`: returns normalized JID of the connected self account (`jidNormalizedUser(socket.user.id)`).
  ...
  - `downloadMedia(msg)` / `downloadMediaFromContent(content)`: download via Baileys `downloadMediaMessage` with fallback to direct `downloadContentFromMessage` stream decryption using `directPath` and `mediaKey` — powers both `.vv` unwrapping and auto-reveal.
  - `getCachedMessage(id)` / `cacheMessage(id, msg)`: in-memory recent-message cache (bounded by `MAX_CACHED_MESSAGES = 300`) backing `.vv` and anti-delete content recovery.
  ...
  - **Deleted Message Deduplication & Detection**: `processedRevokeIds` set prevents duplicate events when both `messages.upsert` (ProtocolMessage REVOKE) and `messages.update` (StubType REVOKE) are emitted for the same message. On revoke detection, builds a `DeletedMessageEvent` from the local cache (`buildDeletedMessageEvent`) and fires all registered `deletedMessageHandlers`.
  ...
  - **Sender PN Cache Bridge**: Short-lived (120s TTL) ID-keyed cache bridging raw CB:message `sender_pn` attribute to normalized messages.
...
- **Note**: `quotedMessage` is declared on `NormalizedMessage` but never populated. `.toaudio`/`.togif` bypass it via the raw `contextInfo` path + `downloadQuotedMedia()`.
...
- **Helper functions** (module-level): `extractViewOnceContent(content)` unwraps `viewOnceMessage`/`viewOnceMessageV2`/`viewOnceMessageV2Extension` wrappers; `getViewOnceMediaType(content)` detects `image`/`video`/`audio` from unwrapped content; `resolveOwnerPhone(client)` resolves the authoritative bot owner phone digits (from Firestore `BOT_OWNER_NUMBER`, env, `getConnectedPhone()`, or LID-to-phone mapping).
- **Pipeline Execution Steps**:
  0. **Auto-view-once reveal** (step 0): When `msg.isViewOnce && !msg.fromMe`, fires `handleAutoVv(msg)` (fire-and-forget). It extracts the view-once content from the raw message, downloads the media using direct decryption stream fallback, and **forwards it strictly and silently to the bot owner's private chat / self-DM ("Subhankar Roy (You)")** via `<ownerDigits>@s.whatsapp.net`. Captions indicate the origin (`from +<sender>` or `from group <groupId> (by +<sender>)`). Writes detailed audit logs (`AUTO_VV_TRIGGER`, `AUTO_VV_SUCCESS`, `AUTO_VV_DOWNLOAD_FAILED`, `AUTO_VV_SEND_FAILED`) to Firestore for full real-time dashboard visibility.
  ...
  4. Determines caller role (`OWNER` if `msg.fromMe` or `isAuthorizedOwner(senderDigits, ownerDigits)` else `PUBLIC`). For group messages, detects WhatsApp group admins via `socket.groupMetadata(msg.chatId)` and promotes the sender to `ADMIN`. Rejects non-owners for `ownerOnly` commands.
  5. Evaluates cooldown via `RateLimiter(5000, 3)` (bypassed for owner self-commands).
  6. Constructs `CommandContext` inline — `reply`/`replyMedia` bound to client sends, plus `replyWithVideo` (raw socket `video` + `gifPlayback`), `replyWithAudio` (raw socket `audio`), `downloadQuotedMedia` (cached original message first, else a synthetic Baileys message built from `contextInfo`), and `getGroupMetadata` (raw socket call) — and executes the plugin via `plugin.execute || plugin.handler`.
...
| **View-Once Handling** | Message parser + recent-message cache + `.vv`/`.avv` command + **auto-view-once reveal** (`handleAutoVv` in dispatcher) | View-once respected by default (`sticker`/`toimg` reject it); `.vv` intentionally reveals a quoted view-once message; **auto-vv silently forwards ALL incoming view-once media strictly to the bot owner's private DM ("Subhankar Roy (You)")** — never revealed in group or sender chat. |
| **Direct Stream Decryption Fallback** | `downloadMediaFromContent` via `downloadContentFromMessage(mediaObj, mediaType)` with `directPath` + `mediaKey` | View-once media with expired or ephemeral CDN URLs is decrypted directly from WhatsApp servers. |
| **Auto-View-Once Forward & Audit** | `handleAutoVv()` in `CommandDispatcher` + Firestore audit logs (`AUTO_VV_TRIGGER`, `AUTO_VV_SUCCESS`, etc.) | Full visibility in Dashboard Audit Logs; incoming view-once media is decrypted and sent to `<ownerDigits>@s.whatsapp.net`. Non-blocking. |
| **Anti-Delete & Revoke Dedup** | `processedRevokeIds` set in `WhatsAppClient` + `messages.upsert`/`messages.update` handlers | Revoked messages are deduplicated so deletions are recorded once; recovered body is preserved for dashboard viewing. |
| **Monetization Access Gate** | `getUserPaymentStatus()` + admin approval on `payments` + `isAdminUser()` auto-approve for OWNER/ADMIN | WhatsApp connect/pairing is blocked until a ₹150 UTR is verified by an admin; admin users bypass payment entirely; approval auto-connects the user's session, rejection/revoke disconnects it immediately. |
